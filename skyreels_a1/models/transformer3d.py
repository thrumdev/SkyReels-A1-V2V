...
class CogVideoXBlock(nn.Module):
    ...
    def __init__(
        ...
        self.attn1 = Attention(
            ...
            processor=FlashAttnCogVideoXAttnProcessor2_0(),
        )
...
class CogVideoXTransformer3DModel(ModelMixin, ConfigMixin, PeftAdapterMixin):
    ...
    def compile_blocks(self):
        for i, block in enumerate(self.transformer_blocks):
            self.transformer_blocks[i] = torch.compile(block)
...
class FlashAttnCogVideoXAttnProcessor2_0:
    r"""
    ...
    def __call__(
        ...
        # [B, H, L, D] -> [B, L, H, D] -> [B, L, 1, H, D]
        query = query.transpose(1, 2).unsqueeze(2)
        key = key.transpose(1, 2).unsqueeze(2)
        value = value.transpose(1, 2).unsqueeze(2)

        # [B, L, 3, H, D]
        qkv = torch.cat([query, key, value], dim=2).to(torch.bfloat16)

        # [B, L, H, D]
        hidden_states = flash_attn_qkvpacked_func(qkv)
